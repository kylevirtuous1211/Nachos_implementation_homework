...
void ExceptionHandler(ExceptionType which) {
    ...
    switch (which) {
        case SyscallException:
            switch (type) {
                ...
                case SC_MSG:
                    ...
                    break;
                case SC_Create:
		    DEBUG(dbgSys, "11111111111.\n");
                    val = kernel->machine->ReadRegister(4);
                    {
			DEBUG(dbgSys, "1111111111122222222222.\n");
                ...
                case SC_Open :
		    DEBUG(dbgSys, "SC_Open 111111111\n");
                    val = kernel->machine->ReadRegister(4);
                    {
			DEBUG(dbgSys, "SC_Open in 222222222\n");
                        char *filename = &(kernel->machine->mainMemory[val]);
                        cout << filename << endl;
                        status = SysOpen(filename);
                        DEBUG(dbgSys, "SC_Open returning status " << status << "\n");
                        kernel->machine->WriteRegister(2, (int)status);
                    }		    
		            kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
                    kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
		...
		case SC_Close :
                    val = kernel->machine->ReadRegister(4);
                    status = SysClose(val);
                    DEBUG(dbgSys, "SC_Close returning status " << status << "\n");
