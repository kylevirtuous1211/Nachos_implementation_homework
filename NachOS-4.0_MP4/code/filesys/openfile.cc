...
int OpenFile::ReadAt(char *into, int numBytes, int position)
{
    ...
    if ((numBytes <= 0) || (position >= fileLength)) {
        DEBUG(dbgFile, "ReadAt: Invalid read request. numBytes: " << numBytes << ", position: " << position << ", fileLength: " << fileLength);
        return 0; // check request
    }
    if ((position + numBytes) > fileLength) {
        numBytes = fileLength - position;
        DEBUG(dbgFile, "ReadAt: Adjusting numBytes to " << numBytes << " to avoid reading beyond file size.");
    }
    DEBUG(dbgFile, "ReadAt: Reading " << numBytes << " bytes at " << position << " from file of length " << fileLength);
    ...
    for (i = firstSector; i <= lastSector; i++) {
        ...
        DEBUG(dbgFile, "ReadAt: Reading sector " << sector);
        kernel->synchDisk->ReadSector(sector, &buf[(i - firstSector) * SectorSize]);
    }
...
int OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    if ((numBytes <= 0) || (position >= fileLength)){
        DEBUG(dbgFile,"numBytes: " << numBytes << " position: " << position << " fileLength: " << fileLength);
        return 0; // check request
    }
    if ((position + numBytes) > fileLength)
        numBytes = fileLength - position;
    ...
    for (i = firstSector; i <= lastSector; i++)
    {
        DEBUG(dbgFile, "Writing sector " << i);
