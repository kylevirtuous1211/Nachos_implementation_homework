...
bool FileHeader::Allocate(PersistentBitmap *freeMap, int fileSize)
{
	...
	//if there is still some data left, allocate a new file header
	if (fileSize > 0) {
	...
	DEBUG(dbgFile, "ALLOCATE COMPLETE");
	DEBUG(dbgFile, "FileHeader::Allocate fileSize: " << FileLength());
	return TRUE;
}
...
//----------------------------------------------------------------------
// FileHeader::CountFileHeaders
// 	Return the number of file headers in the chain.
//----------------------------------------------------------------------
...
{
    printf("\nFileHeader contents.  File size: %d. filehdr count: %d.  Header size: %d.  File blocks:\n", 
           FileLength(),CountFileHeaders() , sizeof(FileHeader)*divRoundUp(FileLength(), MaxFileSize));
    PrintBlock();
    printf("\nFile contents:\n");
    PrintContent();
}
...
void FileHeader::PrintBlock() {
    for (int i = 0; i < numSectors; i++)
        printf("%d ", dataSectors[i]);
    if (nextFileHeader != NULL) nextFileHeader->PrintBlock();
}
...
void FileHeader::PrintContent() {
    ...
    printf("\n");
    delete[] data;
    if (nextFileHeader != NULL) nextFileHeader->PrintContent();
}
